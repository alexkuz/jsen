...
function build(schema, def, additional, followDefaultRefs, resolver) {
    var defType, defValue, key, i, propertyValue;
    ...
    if (defType === 'object' && type(schema.properties) === 'object') {
        for (key in schema.properties) {
            propertyValue = schema.properties[key];

            if (followDefaultRefs &&
                schema.hasOwnProperty('default') &&
                schema.default.hasOwnProperty(key) &&
                schema.default[key].hasOwnProperty('$ref')) {
                propertyValue.$ref = schema.default[key].$ref;
            }

            defValue = build(propertyValue, def[key], additional, followDefaultRefs, resolver);
        ...
        if (type(schema.items) === 'array') {
            for (i = 0; i < schema.items.length; i++) {
                defValue = build(schema.items[i], def[i], additional, followDefaultRefs, resolver);
            ...
            for (i = 0; i < def.length; i++) {
                def[i] = build(schema.items, def[i], additional, followDefaultRefs, resolver);
            }
        }
    }
    ...
    function compile(schema) {
        ...
        compiled.build = function (initial, options) {
            return build(
                ...
                options && options.followDefaultRefs,
                resolver);
        };
